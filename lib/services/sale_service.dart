...
class SaleService {
  ...
  Future<List<Map<String, dynamic>>> getSalesHistory() async {
    final res = await _client
        ...
        .select(
            '*, clientes(nome), itens_venda(quantidade, custo_unitario, preco_unitario, produtos(nome)), parcelas(*)')
  ...
  Future<void> processSale({
    ...
    for (var item in itens) {
      ...
      await _client
          .from(AppTables.produtos)
          .update({'quantidade_estoque': currentStock - item.quantidade}).eq(
              'id', item.produtoId);
    }
  ...
  Future<List<Map<String, dynamic>>> getReceivables() async {
    // Query estendida para buscar as ligações até o nome do produto
    final res = await _client
        .from(AppTables.parcelas)
        .select(
            '*, vendas(cliente_id, clientes(nome, celular), itens_venda(produtos(nome)))')
        .eq('status', 'pendente')
